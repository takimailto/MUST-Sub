...
# ------------------------------------------------------------------------
"""Vision Transformer with dropout 0.1."""
...
from AR.embeddings import PatchEmbed, RotaryEmbed3D
from AR.flex_attention import FlexAttentionCausal2D
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, num_heads, qkv_bias=True, attn_drop=0.1, proj_drop=0.1):
    ...
    def forward(self, x) -> torch.Tensor:
        ...
        if self.flex_attn and self.flex_attn.offsets:
            out = self.flex_attn(q, k, v).transpose(1, 2).flatten(2)
        else:
            o = nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=self.attn_mask)
            out = o.transpose(1, 2).flatten(2)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4, qkv_bias=True, dropout=0.1):
        super(Block, self).__init__()
        self.norm1 = nn.LayerNorm(dim)
        self.attn = Attention(dim, num_heads, qkv_bias=qkv_bias, attn_drop=dropout, proj_drop=dropout)
    ...
    def forward_attn(self, x) -> torch.Tensor:
        return self.norm1(self.dropout(self.attn(x)))

    def forward_mlp(self, x) -> torch.Tensor:
        return self.norm2(self.dropout(self.mlp(x)))
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            Block(embed_dim, num_heads, mlp_ratio, dropout=dropout) for _ in range(depth)
        )
    ...
    def forward_image(self, x, c=None, prev_ids=None, pos=None) -> torch.Tensor:
        x, prev_ids = x if isinstance(x, (tuple, list)) else (x, prev_ids)
        prev_ids = prev_ids if self.encoder_depth else None
        x = x_masked = self.pos_embed(self.patch_embed(x))
        pe1, pe2 = self.prepare_pe(c, prev_ids, pos) if pos is not None else [None] * 2
    ...
    def forward_image_att(self, x, c=None, prev_ids=None, pos=None, layer=0, temperature=1) -> torch.Tensor:
        x = x_masked = self.pos_embed(self.patch_embed(x))
        pe1, pe2 = [None] * 2
        x = x if c is None else torch.cat([c, x], dim=1)
        for blk in self.blocks[: layer]:
            x = blk(x, pe1)
        qkv_shape = [-1, x.size(1), 3, self.blocks[layer].attn.num_heads, self.blocks[layer].attn.head_dim]
        q, k, v = self.blocks[layer].attn.qkv(x).view(qkv_shape).permute(2, 0, 3, 1, 4).unbind(dim=0)
        q, k = (self.blocks[layer].attn.pe_func(q), self.blocks[layer].attn.pe_func(k)) if self.blocks[layer].attn.pe_func else (q, k)
        attn_scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(q.size(-1))
        attention_map = scaled_softmax(attn_scores, temperature=temperature)
        return attention_map
    
    def forward_video_att(self, x, c=None, layer=0, temperature=1) -> torch.Tensor:
        x = x_masked = self.pos_embed(self.patch_embed(x))
        pe1, pe2 = [None] * 2
        x = x if c is None else torch.cat([c, x], dim=1)
        for blk in self.blocks[: layer]:
...
def scaled_softmax(scores, temperature=1.0):
    scores = scores / (temperature * math.sqrt(scores.size(-1)))
