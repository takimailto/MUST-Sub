...
class PatchEmbed(nn.Module):
    ...
    def patchify(self, x) -> torch.Tensor:
        x = x.view(-1, self.image_dim, self.height, self.patch_size, self.width, self.patch_size)  # B 4 16 2 16 2
        return x.permute(0, 2, 4, 3, 5, 1).flatten(1, 2).flatten(2, 4).contiguous()  # B 16 16 2 2 4 # B hw 16

    def unpatchify(self, x) -> torch.Tensor:
        x = x.view(-1, self.height, self.width, self.patch_size, self.patch_size, self.image_dim)  # B 16 16 2 2 4
        return x.permute(0, 5, 1, 3, 2, 4).flatten(2, 3).flatten(3, 4).contiguous()  # B 4 16 2 16 2

    def forward(self, x) -> torch.Tensor:  # B C H W
        flat_shape = (x.size(0), x.size(2)) if x.dim() == 5 else None  # None
        x = x.transpose(1, 2).flatten(0, 1) if x.dim() == 5 else x  # B C H W
        self.width = x.size(-1) // self.patch_size if x.dim() == 4 else self.width  # h
        self.height = x.size(-2) // self.patch_size if x.dim() == 4 else self.height  # w
        x = self.proj(x).flatten(2).transpose(1, 2) if x.dim() == 4 else x  # BT hw C
        return x.view(flat_shape + x.shape[1:]) if flat_shape else x  # B T hw C
...
class MaskEmbed(nn.Module):
    ...
    def forward(self, x) -> torch.Tensor:
        if self.training:
            u_dist = torch.rand(x.shape[:-1] + (1,), device=x.device)  # BT hw 1
            ...
            prev_ids = u_dist.argsort(1)[:, : int(np.round((1 - mask_ratio) * u_dist.size(1)))]  # BT unmasked 1
            self.mask = x.new_ones(u_dist.shape).scatter_(1, prev_ids, 0)  # BT hw 1
            return self.apply_mask(x), prev_ids  # unmasked index
